...
class BERTClassifier(torch.nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        """Forward pass with option to return hidden states."""
        output = self.model(
            input_ids=input_ids,
...
class SecurityAnalyzer:
    ...
    def __init__(
        ...
        self.preprocessor = TextPreprocessor()  # Initialize the preprocessor
    ...
    def calculate_word_attributions(
        ...
        """Calculate word attributions using token attention and hidden states."""
        with torch.no_grad():
            # Get model outputs with hidden states
            outputs, hidden_states = self.model(
                **inputs, 
                return_hidden=True
            )
            
            # Get attention weights from the last layer
            attention_weights = hidden_states[:, 1:].mean(dim=2)  # Skip [CLS]
            
            # Get token ids and create word mapping
            token_ids = inputs['input_ids'][0]
            tokens = self.tokenizer.convert_ids_to_tokens(token_ids)
            
            # Calculate normalized attention scores
            attention_scores = attention_weights[0].cpu()
            attention_scores = (attention_scores - attention_scores.min()) / (
                attention_scores.max() - attention_scores.min() + 1e-6
            )
            
            # Map scores back to original words
            word_scores = {}
            current_word = ""
            current_score = 0.0
            word_count = 0
            
            for token, score in zip(tokens[1:], attention_scores):  # Skip [CLS]
                if token.startswith('##'):
            ...
            # Create final word attributions
            words = text.split()
            word_attributions = []
            ...
            for word in words:
                ...
                score = word_scores.get(word_lower, 0.0)
    ...
    def predict(self, text: str) -> PredictionResult:
        """Make predictions on input text with improved word attributions."""
        try:
            # Tokenize
            ...
            word_attributions = self.calculate_word_attributions(text, inputs)
